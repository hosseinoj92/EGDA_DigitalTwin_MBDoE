...
def mean_rel_error_pct(theta_nat: Dict[str, float],
                       truth: Dict[str, float]) -> float:
    errs = [abs(theta_nat[k] / truth[k] - 1.0) for k in theta_nat
            if k in truth]
...
def plot_error_convergence(results: Dict[str, StrategyResult],
    ...
    for key, res in results.items():
        ...
        e = [mean_rel_error_pct(rec.theta_nat, truth) for rec in res.history]
    ...
    for ip, (pk, ax) in enumerate(zip(pkeys, axes)):
        ...
        for i, key in enumerate(keys):
            ...
            if pk in LOG_KEYS:                 # log-space CI -> asymmetric
                lo = val * (1.0 - np.exp(-1.96 * sig))
                hi = val * (np.exp(1.96 * sig) - 1.0)
            else:
        ...
        for key, res in results.items():
            for rec in res.history:
                ...
                w.writerow([
                    ...
                    f"{mean_rel_error_pct(rec.theta_nat, truth):.3f}"])
...
def write_final_report(results: Dict[str, StrategyResult],
    ...
    lines = ["=" * 74,
             ...
             "=" * 74, "",
             "Hidden truth (revealed post-campaign for benchmarking only):"]
    for pk in pkeys:
