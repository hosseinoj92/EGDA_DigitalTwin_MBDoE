# Graph Report - Homogenous  (2026-08-06)

## Corpus Check
- 74 files · ~219,274 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 872 nodes · 1949 edges · 48 communities (46 shown, 2 thin omitted)
- Extraction: 90% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 184 edges (avg confidence: 0.72)
...
1. `OperatingConditions` - 50 edges
2. `Layer1Bridge` - 38 edges
3. `ParameterSpace` - 33 edges
4. `InferenceModel` - 30 edges
5. `NoiseModel` - 26 edges
6. `run()` - 25 edges
7. `ParallelConfig` - 24 edges
8. `run_analysis()` - 23 edges
9. `PFRResult` - 23 edges
10. `MBDoESelector` - 23 edges
...
Cohesion: 0.16
Nodes (25): _coverage(), _invalid_records(), Any, Path, run_analysis(), functional_anova(), geometry_collapse(), _groups() (+17 more)
...
### Community 4 - "OperatingConditions"
Cohesion: 0.12
Nodes (24): Seven-step self-driving closed loop, Bounded continuous Powell refinement of the best candidate, Measurement object (condition, ports, species, noisy values), noise_true vs noise_assumed misspecification study hook, OperatingConditions experiment record, Recommended study extensions (Monte Carlo, cost-aware, ablation), Closed-loop campaign runner - the "self-driving" part. For one strategy the…, RoundRecord (+16 more)
...
Cohesion: 0.09
Nodes (38): Truth/inference firewall, Truth-only systematic effects (transfer_time_s, calibration_gain), build_candidates(), build_fixed_design(), Full-factorial candidate grid over the feasible design space., Conventional campaign: temperature ladder at nominal flow/catalyst. `budget`…, literature_guess(), Initial estimates = Layer 1's literature-anchored kinetics for the chosen… (+30 more)

### Community 9 - "run_temperature_study.py"
Cohesion: 0.12
Nodes (29): main(), BATCH temperature studies of the PFR digital twin. Same physics and outputs as…, Index table plus the cross-scenario comparison figures (each + CSV)., Sweep every scenario, write per-scenario folders and the summary., run_batch(), _write_summary(), make_run_dir(), _num() (+21 more)
...
### Community 11 - "run_simulation.py"
Cohesion: 0.18
Nodes (17): analytical_profiles(), max_relative_error(), ndarray, Algebraic reference solutions used to verify the numerical integrator. 1.…, Largest |numerical - analytical| across species, relative to `scale`., Concentration-based reaction quotients (Q1, Q2) of the two steps; NaN where the…, reaction_quotients(), build_inlet() (+9 more)

### Community 12 - "reactor.py"
Cohesion: 0.13
Nodes (15): _bracketed_root(), equilibrium_state(), Composition at simultaneous chemical equilibrium of both steps. Solves for the…, Root of a monotonically increasing f on [lo, hi] with f(lo) <= 0 <= f(hi)., Straight cylindrical tube; defaults match the 200 mm x 18 mm ID lab PFR., ReactorGeometry, SolverSettings, nu_matrix() (+7 more)

### Community 13 - "speciation.py"
Cohesion: 0.06
Nodes (46): Axial EGMA peak location and 95% plateau interval, Damköhler kinetic-exposure metric, bisulfate_equilibrium(), [H+] for total sulfate molarity c_total, ideal activities (back-compat alias…, aphi(), bisulfate_dilute(), bisulfate_pitzer(), _g() (+38 more)

### Community 14 - "InferenceModel"
Cohesion: 0.22
Nodes (6): InferenceModel, ndarray, Central-difference S (m.size x p) in scaled parameter space., The self-contained, picklable recipe for scoring one candidate. Carries no…, Expected FIM contribution of a candidate experiment, evaluated at the CURRENT…, Re-estimate theta from all accumulated data (warm start).
...
### Community 17 - "parallel.py"
Cohesion: 0.15
Nodes (26): Executor, _chunk_information(), condition_information(), condition_information_fanned(), _contiguous_chunks(), _fd_vectors(), FIMSpec, _get_executor() (+18 more)
...
### Community 20 - "AnalysisTests"
Cohesion: 0.24
Nodes (5): AnalysisTests, Path, sample_payload(), sample_profile(), write_scenario()
...
### Community 23 - "Layer1Bridge"
Cohesion: 0.09
Nodes (26): greedy_d_optimal(), information_matrices(), _logdet_floored(), ndarray, Pre-campaign identifiability screen. Before a single experiment is spent, ask…, Per-condition information M_e = S_e' Sigma_e^-1 S_e at `theta_vec` (default:…, log det F with eigenvalues floored, so a rank-deficient F still ranks sensibly…, FIM of the greedy D-optimal `budget`-experiment design over `mats`. (+18 more)
...
### Community 29 - "run"
Cohesion: 0.43
Nodes (6): main(), Layer 2 showcase: virtual self-driving laboratory around the Layer 1 PFR twin.…, resolve_outdir(), run(), Corners (plus centre) of the admissible box - the most informative design the…, reference_design()
...
Cohesion: 0.36
Nodes (4): MBDoESelector, ndarray, Design score of every coarse candidate, IN CANDIDATE ORDER. This is the…, Design score with FLOORED eigenvalues. `slogdet` returns -inf for any singular…
...
### Community 34 - "surrogate_validation"
Cohesion: 0.25
Nodes (8): _compiled_nondominated_indices(), _error_metrics(), _polynomial_design(), ndarray, Validate one quadratic response surface per study. All requested responses…, Compiled incremental skyline scan for large exact fronts., surrogate_validation(), njit

### Community 35 - "create_figures"
Cohesion: 0.52
Nodes (6): create_figures(), _finish(), Any, Figure, Path, _study_groups()

### Community 36 - "pfr_twin/__init__.py"
Cohesion: 0.08
Nodes (30): flow_diagnostics(), Plug-flow validity diagnostics. A digital twin should say when its own…, Vogel-type correlation for liquid water, valid ~273-373 K., water_density_g_L(), water_viscosity_Pa_s(), pfr_twin - 1D deterministic digital twin of an isothermal plug flow reactor for…, Kinetic model of the two-step series ester cleavage, per catalyst system. Acid…, InletState (+22 more)
...
### Community 47 - "sdl/parameters.py"
Cohesion: 0.50
Nodes (3): The inverse problem — kinetics from noisy measurements, Reference-temperature (k_ref, Ea) reparameterization, Estimation parameter space (catalyst-aware). Natural parameters (dict keys,…
...
- **What is the exact relationship between `NaOH_exhausted Regime` and `Finding: NaOH Surrogate Error ~4x Larger Than H2SO4`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `default_kinetics()` connect `pfr_twin/__init__.py` to `self_test.py`, `run_simulation.py`, `speciation.py`, `test_analysis.py`?**
